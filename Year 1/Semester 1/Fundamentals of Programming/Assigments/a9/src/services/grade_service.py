...
class GradeService():
    def __init__(self,  repository, undo_service):
        if get_repository_name() == "Memory":
            ...
            self._repo = repository("grades.txt")
        elif get_repository_name() == "Binary":
            self._repo = repository("grades.bin")
    ...
    def check_if_grade_is_valid(self, grade):
        self._repo.check_if_input_is_valid_grades(grade)

    def add_grade(self, discipline_id, student_id, grade_value):
        new_grade = Grade(discipline_id, student_id, grade_value)
        self._repo.add_new_grade(new_grade)

        UndoCommand = CommandThatCanBeCalled(self._repo.remove_grade, (new_grade))
        RedoCommand = CommandThatCanBeCalled(self._repo.add_new_grade, new_grade)
    ...
    def get_all_grades(self):
        return self._repo.get_all_grades()
